<x-jet-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
            <div x-data="{nombreFoto: null, vistaPreviaFoto: null}" class="col-span-6 sm:col-span-4">
                <!-- Entrada de archivo para la foto de perfil -->
                <input type="file" class="hidden"
                            wire:model="photo"
                            x-ref="photo"
                            x-on:change="
                                    nombreFoto = $refs.photo.files[0].name;
                                    const lector = new FileReader();
                                    lector.onload = (e) => {
                                        vistaPreviaFoto = e.target.result;
                                    };
                                    lector.readAsDataURL($refs.photo.files[0]);
                            " />

                <x-jet-label for="photo" value="{{ __('Foto') }}" />

                <!-- Foto de perfil actual -->
                <div class="mt-2" x-show="! vistaPreviaFoto">
                    <img src="{{ $this->user->profile_photo_url }}" alt="{{ $this->user->name }}" class="rounded-full h-20 w-20 object-cover">
                </div>

                <!-- Vista previa de la nueva foto de perfil -->
                <div class="mt-2" x-show="vistaPreviaFoto" style="display: none;">
                    <span class="block rounded-full w-20 h-20 bg-cover bg-no-repeat bg-center"
                          x-bind:style="'background-image: url(\'' + vistaPreviaFoto + '\');'">
                    </span>
                </div>
